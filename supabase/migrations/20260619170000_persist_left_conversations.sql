...
-- ============================================================
-- Update: list_my_conversations to include conversations
-- that the user has left, but only if they are not hidden.
-- ============================================================
...
BEGIN
  RETURN QUERY
  WITH settings AS (
    -- Get settings for the user's conversations
  ...
  my_convs AS (
    -- Get all conversations where:
    -- 1. User is a participant
    -- 2. User HAS NOT left OR (User HAS left AND Chat is NOT hidden)
    SELECT c.id, c.user1_id, c.user2_id, c.last_message_at
    FROM public.conversations c
    JOIN public.conversation_status cs ON cs.conversation_id = c.id
    LEFT JOIN settings s ON s.conversation_id = c.id
    WHERE cs.user_id = v_user_id 
      AND (cs.has_left = false OR COALESCE(s.is_hidden, false) = false)
  ),
  last_msgs AS (
