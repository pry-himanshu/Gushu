-- ============================================================
-- Add removed_at to conversation_settings and update RPC
-- ============================================================
...
-- 2. Drop existing function to allow signature change
...
BEGIN
  ...
  WHERE mc.id IS NOT NULL
    AND (
      -- Always include hidden chats in RPC (management view needs them)
      COALESCE(s.is_hidden, false) = true
      OR 
      -- Show if it hasn't been removed, OR if there is new activity,
      -- OR if there are UNREAD messages (safety override for disappearance)
